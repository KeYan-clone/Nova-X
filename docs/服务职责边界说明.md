...
### monitor-service - 指标监控告警
**单一职责**：实时性能监控与告警
- ✅ 负责：Prometheus 指标采集、实时告警、性能分析
- ✅ 负责：健康检查聚合、SLA 监控
- ❌ 不负责：日志存储检索（由 log-service 负责）
- ❌ 不负责：事后问题分析（由 log-service 日志检索支持）
...
### common-log vs log-service

| 功能         | common-log（采集端） | log-service（聚合端） |
| ------------ | -------------------- | --------------------- |
| TraceID 生成 | ✅                    | ❌                     |
| 日志切面     | ✅                    | ❌                     |
| 日志拦截器   | ✅                    | ❌                     |
...
应用日志链路：
  ...
  6. log-service 从 Kafka 消费并写入 Elasticsearch
  7. log-service 提供检索 API
```
