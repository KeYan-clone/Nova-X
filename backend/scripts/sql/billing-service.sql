...
CREATE TABLE `bill` (
  ...
  `bill_status` varchar(20) NOT NULL DEFAULT 'UNPAID' COMMENT '账单状态',
...
-- 3. 插入测试数据
INSERT INTO `bill` (`bill_no`, `session_id`, `user_id`, `station_id`, `station_name`, `device_id`, `device_code`, `connector_id`, `connector_code`, `start_time`, `end_time`, `charging_duration`, `charging_energy`, `pricing_template_id`, `electricity_cost`, `service_fee`, `parking_fee`, `discount_amount`, `total_amount`, `paid_amount`, `bill_status`)
VALUES
('BL20260120120000001', 1, 1001, 1, '市中心充电站', 1, 'DV20260120001', 1, 'CNDV20260120001001', '2026-01-20 08:00:00', '2026-01-20 10:30:00', 150, 35.68, 1, 28.54, 3.57, 5.00, 0.00, 37.11, 37.11, 'PAID');
...
-- 5. 定价规则表 (修正重点：补齐了表名和 operator_id)
DROP TABLE IF EXISTS `t_pricing_rule`;
CREATE TABLE `t_pricing_rule` (
    `id` BIGINT PRIMARY KEY AUTO_INCREMENT,
    `operator_id` BIGINT NOT NULL COMMENT 'Operator ID',
    `station_id` BIGINT DEFAULT NULL COMMENT 'Station ID (NULL for default)',
...
-- 6. 计费明细表
DROP TABLE IF EXISTS `t_billing_item`;
